...
//  instance.  When the clock hits `arrivalTime`, the main simulation loop (os.cpp) constructs a      //
//  live 'Process' object (see PCB.h) by copying these fields from a "processData.h" instance.        //
//  It then initializes the run-time state for the newly created PCB before the scheduler places it   //
//  in the ready queue.                                                                               //
//  Separating ProcessData from the PCB provides two big advantages:                                  //
